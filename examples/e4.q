...
if[0i=first .r.get"is.element(\"zoo\",installed.packages()[,1])";
   .r.exec"install.packages(\"zoo\",repos=\"https://cloud.r-project.org\")"]
.r.exec"library(zoo)";
...
.r.set[`d;d]
.r.set[`pr;pr]
.r.exec"p <- structure(pr, index = d, frequency = 0.142857142857143, class = c(\"zooreg", "zoo\"))";

// Shift to returns --
.r.exec"r <- 100*diff(log(p))";
.r.get"head(r)"
.r.get"summary(r)"
.r.get"sd(r)"

// Compute the moving window vol --
.r.exec"vol <- sqrt(250) * rollapply(r, 20, sd, align = \"right\")";

// A pretty plot --
.r.exec"png('pretty_plot_test.png')";
.r.exec"plot(vol, type=\"l\", ylim=c(0,max(vol,na.rm=TRUE)),lwd=2, col=\"purple\", xlab=\"2004\",ylab=paste(\"Annualised sigma, 20-week window\"))";
.r.exec"grid()";
.r.get"legend(x=\"bottomleft\", col=c(\"purple\", \"darkgreen\"),lwd=c(2,2), bty=\"n\", cex=0.8,legend=c(\"Annualised 20-week vol (left scale)\", \"Nifty (right scale)\"))"
.r.exec"par(new=TRUE)";
.r.exec"plot(p, type=\"l\", lwd=2, col=\"darkgreen\", xaxt=\"n\", yaxt=\"n\", xlab=\"\", ylab=\"\")";
.r.exec"axis(4)";
.r.off[]
