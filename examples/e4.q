...
if[0i=first .rk.get"is.element(\"zoo\",installed.packages()[,1])";
   .rk.exec"install.packages(\"zoo\",repos=\"https://cloud.r-project.org\")"]
.rk.exec"library(zoo)";
...
.rk.set[`d;d]
.rk.set[`pr;pr]
.rk.exec"p <- structure(pr, index = d, frequency = 0.142857142857143, class = c(\"zooreg", "zoo\"))";

// Shift to returns --
.rk.exec"r <- 100*diff(log(p))";
.rk.get"head(r)"
.rk.get"summary(r)"
.rk.get"sd(r)"

// Compute the moving window vol --
.rk.exec"vol <- sqrt(250) * rollapply(r, 20, sd, align = \"right\")";

// A pretty plot --
.rk.exec"png('pretty_plot_test.png')";
.rk.exec"plot(vol, type=\"l\", ylim=c(0,max(vol,na.rm=TRUE)),lwd=2, col=\"purple\", xlab=\"2004\",ylab=paste(\"Annualised sigma, 20-week window\"))";
.rk.exec"grid()";
.rk.get"legend(x=\"bottomleft\", col=c(\"purple\", \"darkgreen\"),lwd=c(2,2), bty=\"n\", cex=0.8,legend=c(\"Annualised 20-week vol (left scale)\", \"Nifty (right scale)\"))"
.rk.exec"par(new=TRUE)";
.rk.exec"plot(p, type=\"l\", lwd=2, col=\"darkgreen\", xaxt=\"n\", yaxt=\"n\", xlab=\"\", ylab=\"\")";
.rk.exec"axis(4)";
.rk.off[]
