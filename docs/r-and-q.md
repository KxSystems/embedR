...
The first and second methods on interfacing between q and R are covered by the Fusion interfaces embedR (this repo) and [rkdb](https://github.com/KxSystems/rkdb). The [other methods](other-methods.md) are not supported or owned by KX; the packages and methods are kdb-Rmath, RServe and RODBC.
